...
| Haute | 2 findings (accepted) |
| Moyenne / Basse / Info | 0 |

**Verdict: an unauthenticated RCE path was found and verified** (`preuve statique forte`, every transition accepted). A remote anonymous attacker `POST`s `{"is_admin": true}` to `/login` (mass assignment → admin session), then replays the cookie to `/render` where attacker `tpl` is rendered as a Jinja2 template (SSTI) → server-side code execution.
...
Chain accepted, both members accepted, entry unauthenticated, impact unauth-rce → **Critique**.

---

## Detailed findings

### OSWE-1 — Privilege escalation via mass assignment of session admin flag — Haute (accepted, `preuve statique forte`)
...
### OSWE-2 — SSTI in `/render` via `render_template_string` → RCE — Haute (accepted, `preuve statique forte`)
...
- **Verification:** target set = OSWE-1, OSWE-2 (both Haute + chain members) + CHAIN-1 → 2 batches; both passed `validate-batch` and the `apply-verdicts` preflight (`ok:true`). **No gaps, no neutralized batches, no `not-requested` targets.**
