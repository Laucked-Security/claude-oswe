...
1.000** over Semgrep, refuting Semgrep's false positives while keeping a complete proof for each promotion.

**Trust-boundary hygiene lane** (SP7). On top of the RCE-chain core, obvious CWE-501 trust-boundary
violations (attacker data written into trusted/session state) are reported as **Low/Info hygiene
findings** — never a chain member, never an RCE path, accepted by the verifier only for a direct
attacker→trusted-store crossing. This recovered the 8 previously-missed trustbound cases (recall
0.929 → 1.000) without adding a single false positive. *OSWE-core for exploit chains, plus explicit
low-severity hygiene findings when the evidence is clean.* See [`benchmark/BENCHMARK.md`](benchmark/BENCHMARK.md).
